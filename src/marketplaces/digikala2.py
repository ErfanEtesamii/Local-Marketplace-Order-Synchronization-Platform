"""
Digikala adapter - SECOND STORE.

This account has two separate Digikala seller stores/panels; this module
is the adapter for the second one, kept as a fully independent module
(own class `Digikala2Adapter`, own config `settings.digikala2`, own
watermark row, own token-cache file - data/digikala2_tokens.json) rather
than sharing a class with the first store's adapter
(src/marketplaces/digikala.py) - client request, 2026-09: treat this as
its own marketplace integration, the same way Tapsi Shop/Basalam/Faraz
Honar/SnappShop each get their own file, not as a parameterized instance
of an existing one.

Concretely this means the content below is a maintained COPY of
digikala.py's logic, not an import/subclass of it - the two files can
and will drift independently over time (e.g. a bugfix landing in one
does NOT automatically apply to the other; each needs to be applied to
both files if it should apply to both stores). This is a deliberate
trade-off the client asked for in exchange for the two stores being
completely decoupled, unlike the original config-parameterized "one
shared class" approach.

Everything below - the Open API shape, the 2026-09 SBS migration
(shipmentId watermark instead of a created_at window), the pending-order
auto-confirm flow, the token refresh lifecycle, the /orders/history price
enrichment - is IDENTICAL to the first store's own API/behavior, since
both stores talk to the exact same Digikala Open API. See
src/marketplaces/digikala.py's own module docstring for the full
detail/history behind each of those, all of which applies here unchanged
except for which store's credentials/watermark/token-cache it operates
on.
...
class Digikala2Adapter(MarketplaceAdapter):
    ...
    # module's docstring. Hardcoded (not derived from config) since this
    # module only ever represents this one store.
    name = "digikala2"
    ...
    def __init__(
        ...
        # Defaults to this store's own settings.digikala2 (DIGIKALA2_* env
        # vars - see src/config.py) - NOT settings.digikala, which belongs
        # to the first store's own adapter (src/marketplaces/digikala.py).
        self._config = config or settings.digikala2
        ...
        # `config or settings.digikala2` just above). The watermark table
        # is keyed by platform name ("digikala2"), so this never collides
        # with the first store's own "digikala" watermark row even though
        # both adapters share the same Repository/db file.
        self._repo = repository or Repository()
        # Own token-cache file, entirely separate from the first store's
        # data/digikala_tokens.json - this store's refreshed access/
        # refresh tokens are never written to or read from that file.
        self._token_cache_path = Path(settings.db_path).resolve().parent / "digikala2_tokens.json"
        ...
        # 2026-09 discount-enrichment fix, mirrored from digikala.py (see
        # that file's DigikalaAdapter.__init__ / _fetch_variant_promotion
        # for the full rationale - this module is a maintained copy, per
        # this file's own module docstring, so the fix is applied here
        # too rather than only to the first store).
    ...
    def _load_tokens(self) -> tuple[str, str]:
        ...
        if self._token_cache_path.exists():
            try:
                ...
                log.warning(
                    "digikala2: failed to read cached tokens at %s, falling back to .env",
    ...
    def _refresh_if_expiring(self) -> None:
        ...
        if self._adopt_cached_tokens():
            ...
            if left is None or left > lead:
                log.info(
                    "digikala2: adopted a fresher access token from the shared cache "
                    "(expires in %d s)",
        ...
        if time.monotonic() < self._next_proactive_refresh_at:
            return
        log.info(
            "digikala2: access token expires in %d s (lead %d s), refreshing proactively",
            int(left),
            lead,
        )
        try:
            ...
            log.warning(
                "digikala2: proactive token refresh failed (%s); continuing with the "
    ...
    def _refresh_access_token(self) -> None:
        ...
        self._save_tokens()
        log.info(
            "digikala2: access token refreshed, new expiry=%s",
    ...
    def _get(self, path: str, params: dict, _already_refreshed: bool = False) -> dict:
        ...
        if resp.status_code == 401 and not _already_refreshed:
            if self._adopt_cached_tokens(current_known_bad=True):
                log.info(
                    "digikala2: access token rejected (401) but the shared token cache "
                    "already holds a newer pair, using it instead of refreshing"
                )
            else:
                log.info("digikala2: access token expired (401), refreshing")
                self._refresh_access_token()
    ...
    def _update_status(
        ...
        if resp.status_code == 401 and not _already_refreshed:
            if self._adopt_cached_tokens(current_known_bad=True):
                log.info(
                    "digikala2: access token rejected (401) during update-status but the "
                    "shared token cache already holds a newer pair, using it"
                )
            else:
                log.info("digikala2: access token expired (401) during update-status, refreshing")
                self._refresh_access_token()
    ...
    def fetch_new_orders(self, since: datetime | None) -> list[NormalizedOrder]:
        ...
        if watermark is None:
            ...
            log.info(
                "digikala2: cold start - seeded shipment watermark at %d, no orders synced",
                seeded,
            )
            return []
        ...
        # Promotions API - mirrored from digikala.py, see
        # _fetch_variant_promotion's docstring there for the full
        # investigation of why /orders/history was confirmed unusable
        # for SBS orders). One best-effort lookup per row, done here
        # (the I/O-performing entry point) rather than inside
    ...
    def _confirm_if_pending(self, row: dict) -> dict:
        ...
        try:
            ...
            log.exception(
                "digikala2: failed to auto-confirm pending shipment %s (new_status=%s) - "
        ...
        try:
            ...
            log.exception(
                "digikala2: confirmed shipment %s but failed to re-fetch its detail - "
        ...
        if not refreshed:
            log.warning(
                "digikala2: confirmed shipment %s but re-fetch returned no data - "
                "syncing with the pre-confirmation row",
                shipment_id,
            )
            return row
        ...
        log.info(
            "digikala2: auto-confirmed pending shipment %s -> %s (customer data present=%s)",
    ...
    def fetch_sbs_customer_details(self, shipment_id: str) -> dict:
        ...
        try:
            ...
            log.info(
                "digikala2: fetched SBS customer details for shipment %s "
            ...
            log.exception(
                "digikala2: failed to fetch SBS customer details for shipment %s",
                shipment_id,
            )
            return empty
    ...
    def fetch_shipment_details(self, shipment_id: str) -> dict:
        ...
        try:
            ...
            if not items:
                log.warning(
                    "digikala2: no items in shipment details for shipment %s", shipment_id,
                )
            ...
            log.info(
                "digikala2: fetched shipment details for shipment %s "
            ...
            log.exception(
                "digikala2: failed to fetch shipment details for shipment %s", shipment_id,
            )
            return {"tracking_code": None, "shipping_cost": None}
    ...
    def _fetch_variant_promotion(self, variant_id: int | str | None) -> dict | None:
        """
        2026-09 discount-enrichment fix - mirrored from digikala.py's
        DigikalaAdapter._fetch_variant_promotion. See that method's
        docstring for the full investigation trail (both stores talk to
        the same Digikala Open API, so the same finding applies here:
        /orders/history is confirmed unusable for SBS orders, and SBS's
        own variants[].price is already the post-discount value).

        Returns {"rrp_price": Decimal, "selling_price": Decimal} for the
        first APPROVED current promotion on this variant, or None if
        there's no active promotion right now (common, not an error) or
        the lookup failed. Never raises. Cached per variant_id for this
        adapter instance's lifetime (own cache, separate from the first
        store's - see self._variant_promotion_cache in __init__).
        """
        ...
        try:
            ...
            log.exception(
                "digikala2: failed to fetch current promotions for variant %s - "
    ...
    def _build_promotion_map(self, row: dict) -> dict[int, dict]:
        ...
        promotion - mirrored from digikala.py's
        DigikalaAdapter._build_promotion_map.
        """
    ...
    def _fetch_history_price_map(self, order_id, order_date: str | None) -> dict[str, dict]:
        ...
        digikala.py's DigikalaAdapter._fetch_variant_promotion docstring
        for the full investigation: this endpoint was confirmed live to
        have a 100% miss rate for SBS orders, and is now replaced by the
        Promotions API in both stores. Left in place rather than deleted
        for the same reason as in digikala.py - see that file's own note
        next to its (identical) _fetch_history_price_map.
        ...
        try:
            while True:
                ...
                if page >= 20:
                    log.warning(
                        "digikala2: history price lookup for order %s hit the 20-page "
            ...
            log.exception(
                "digikala2: failed to fetch /orders/history price data for order %s - "
        ...
        if not price_map:
            log.warning(
                "digikala2: no matching /orders/history rows found for order %s within "
    ...
    def _normalize_sbs_row(self, row: dict, promotion_map: dict[int, dict] | None = None) -> NormalizedOrder:
        ...
        # THIS endpoint. ALSO CONFIRMED (2026-09, mirrored from
        # digikala.py's own finding, order 383370810 / product code
        # 3818): this "price" is itself ALREADY the post-discount
        # selling price, not the original - so it's usable as
        # final_price, but never as unit_price when a discount is
        # active. Callers pass in `promotion_map` - looked up from the
        # Promotions API via _fetch_variant_promotion()/
        # _build_promotion_map(), keyed by each item's own `variantId` -
        # for the real original (pre-discount) price. A missing/failed
        # lookup is safe: promotion_map={} just falls back to the
        # price*count/no-discount branch below, unchanged from before
        # this enrichment existed (which itself was already the
        # behavior whenever no promotion happens to be active on a
        # given variant - not an error case).
