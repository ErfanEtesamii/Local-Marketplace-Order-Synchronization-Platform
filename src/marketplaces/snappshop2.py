"""
SnappShop adapter - SECOND VENDOR ACCOUNT.

This account has two separate SnappShop vendor accounts; this module is
the adapter for the second one, kept as a fully independent module (own
class `SnappShop2Adapter`, own config `settings.snappshop2`) rather than
sharing a class with the first account's adapter
(src/marketplaces/snappshop.py) - client request, 2026-09 chat: "اسنپ
شاپ دوم رو هم مشابه دیجی کالا دوم میخوایم ثبت کنیم" (register a second
SnappShop the same way as the second Digikala). Same
own-class/own-file/own-config decoupling tradeoff as
src/marketplaces/digikala2.py (see that module's docstring for the full
rationale) - a maintained COPY of snappshop.py's logic, not an
import/subclass of it, so the two files can and will drift
independently over time.

ONE DELIBERATE DIFFERENCE FROM digikala2.py: the second Digikala store
got its OWN Didar Deal Label ("دیجی کالا سریع"), but this second
SnappShop account explicitly reuses the FIRST store's own Deal Label
("اسنپ") - a specific client instruction for this account, not the
general pattern for a second store. See src/config.py's
`deal_label_title_by_source` property, which resolves the
"snappshop2" key straight to `deal_label_title_snappshop` rather than
a distinct field.

There is no watermark/token-cache subtlety to carry over here the way
there was for digikala2: this adapter (like the first store's own
snappshop.py) has no token-refresh lifecycle and uses SyncEngine's
regular created_at-based polling, not an id-based watermark - the
config/wiring differences vs. the first store are limited to which
vendor account's credentials (`SNAPPSHOP2_*` env vars via
`settings.snappshop2`) this instance talks to.

Everything below - the Open API shape, auth headers, pagination,
normalization - is IDENTICAL to the first account's own API/behavior,
since both accounts talk to the exact same SnappShop vendor API. See
src/marketplaces/snappshop.py's own module docstring for the full
detail/history behind each of those, all of which applies here
unchanged except for which vendor account's credentials it operates on.
...
This second account's `SNAPPSHOP2_ENABLED` flag (see
`_build_snappshop2_config` in src/config.py) tracks whether THIS
account's own credentials (vendor_id above all - see .env.example) are
in place, same opt-in-until-ready pattern as `SNAPPSHOP_ENABLED` for
the first account. Flip it to `true` once `SNAPPSHOP2_VENDOR_ID` (and
the auth token / agent user) are confirmed; no further code change is
needed to trust the output at that point - schema confidence is shared
with the first account since both talk to the identical API.
"""
...
class SnappShop2Adapter(MarketplaceAdapter):
    name = "snappshop2"

    def __init__(self, config: SnappShopConfig | None = None) -> None:
        self._config = config or settings.snappshop2
    ...
    def fetch_new_orders(self, since: datetime | None) -> list[NormalizedOrder]:
        if not _SCHEMA_CONFIRMED:
            ...
            log.warning(
                "snappshop2: order field schema was rewritten against the official "
                "v2.1.2 vendor-API PDF but has not yet been verified against a real "
                "populated response - see src/marketplaces/snappshop2.py module "
        ...
        log.info("snappshop2: fetched %d orders", len(orders))
        return orders
    ...
    def discover_vendor_id(self) -> str:
        ...
        if not vendors:
            raise ValueError("snappshop2: GET /vendors returned no vendors")
    ...
    def _normalize_items(self, raw_items: list[dict]) -> list[OrderItem]:
        ...
        for item in raw_items:
            ...
            if item_status == "CANCELED":
                ...
                log.debug(
                    "snappshop2: dropping fully-canceled item (product_number=%s)",
...
def _parse_date(value: str | None) -> datetime:
    ...
    try:
        ...
        # BUGFIX (2026-09-21): mirrors the same fix in snappshop.py (this
        # store is a maintained independent copy) - SnappShop doesn't
        # always send an offset/"Z" suffix on created_at, and an
        # offset-naive result here crashes sync_engine.py's window
