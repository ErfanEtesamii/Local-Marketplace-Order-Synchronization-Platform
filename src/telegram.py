...
class TelegramNotifier:
    ...
    def _format_new_order_message(self, order) -> str:
        ...
        # (Digikala, then Faraz Honar - see src/shipping_fees.py's
        # module docstring for the history). shipping_fee_rial() now
        # always returns None, so this always falls through to the
        # real order.shipping_cost in Rial, with order.total_price as
        # the grand total - same real figures every source's adapter
        # already reads from its own API.
