...
from src.marketplaces.digikala2 import Digikala2Adapter, _IRAN_TZ
...
def _empty_history_response():
    ...
    (see Digikala2Adapter._fetch_history_price_map) - an empty result,
    ...
    below (SUPERSEDED but kept - see its own docstring in
    digikala2.py); this helper is still used by those direct tests
    only, no longer by fetch_new_orders/fetch_order_detail - see
    _mock_no_promotions() below for their current blanket mock."""
...
# from /orders/history to the Promotions API - mirrored from
# test_digikala.py, see digikala2.py's _fetch_variant_promotion docstring.
...
def test_cold_start_seeds_watermark_without_syncing_anything(repo):
    ...
    adapter = Digikala2Adapter(config=_CFG, repository=repo)
    orders = adapter.fetch_new_orders(since=None)

    assert orders == []
    assert repo.get_last_shipment_id("digikala2") == 777
...
def test_cold_start_seeds_zero_when_account_has_no_shipments(repo):
    ...
    assert orders == []
    assert repo.get_last_shipment_id("digikala2") == 0
...
def test_fetch_new_orders_uses_watermark_plus_one_and_advances_it(repo):
    ...
    repo.set_last_shipment_id("digikala2", 100)
    _mock_no_promotions()
    route = respx.get(_SBS_URL).mock(
    ...
    assert repo.get_last_shipment_id("digikala2") == 105
...
def test_fetch_new_orders_returns_nothing_when_no_new_shipments(repo):
    ...
    repo.set_last_shipment_id("digikala2", 500)
    respx.get(_SBS_URL).mock(return_value=_sbs_list_response([]))
    ...
    assert orders == []
    assert repo.get_last_shipment_id("digikala2") == 500
...
def test_watermark_persisted_after_every_page_not_just_at_the_end(repo):
    ...
    repo.set_last_shipment_id("digikala2", 0)
    _mock_no_promotions()
    ...
    def _responder(request):
        ...
        seen_watermark_before_page_2["value"] = repo.get_last_shipment_id("digikala2")
    ...
    assert repo.get_last_shipment_id("digikala2") == 51
...
def test_since_argument_is_ignored(repo):
    ...
    assert repo.get_last_shipment_id("digikala2") is not None
...
def test_expired_access_token_triggers_refresh_and_retry(tmp_path, repo):
    ...
    repo.set_last_shipment_id("digikala2", 100)  # skip cold-start seeding for this test
    adapter = Digikala2Adapter(config=cfg, repository=repo)
    adapter._token_cache_path = tmp_path / "digikala2_tokens.json"  # isolate from real cache
...
def test_refreshed_tokens_are_persisted_and_reused_on_restart(tmp_path, repo):
    ...
    """
    cache_path = tmp_path / "digikala2_tokens.json"
    ...
    restarted = Digikala2Adapter(config=cfg, repository=repo)
...
def test_normalize_sbs_row_matches_client_reported_example(repo):
    """Regression test mirrored from the first store's test suite - same
    real-world case (product code 3818, 5% discount), confirmed to
    apply identically to this store since both talk to the same
    Digikala Open API."""
    adapter = Digikala2Adapter(config=_CFG, repository=repo)
    row = _sbs_row(
...
def test_normalize_sbs_row_clamps_final_price_when_discount_exceeds_unit_price(repo):
    """Bad/stale promotion data must never produce a nonsensical figure -
    final_price now comes straight from promotion_selling_price rather
    than a subtraction, so there's nothing left to clamp here (see
    test_digikala.py's identical test for the full rationale)."""
    adapter = Digikala2Adapter(config=_CFG, repository=repo)
    row = _sbs_row(
        shipment_id=1,
        variants=[
...
def test_fetch_new_orders_ignores_non_approved_promotions(repo):
    ...
    ignored, falling back to price*count."""
    repo.set_last_shipment_id("digikala2", 0)
    respx.get(_SBS_URL).mock(
        return_value=_sbs_list_response(
            [
...
def test_fetch_new_orders_caches_promotion_lookup_across_rows(repo):
    ...
    poll cycle must only trigger ONE Promotions API call."""
    repo.set_last_shipment_id("digikala2", 0)
    respx.get(_SBS_URL).mock(
        return_value=_sbs_list_response(
            [
