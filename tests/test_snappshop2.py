"""
Tests for the SnappShop adapter - SECOND vendor account.

Mirrors test_snappshop.py exactly (same confirmed v2.1.2 API schema,
same worked samples - see src/marketplaces/snappshop2.py's module
docstring for why this is a maintained copy of the first account's
adapter/tests rather than a shared/parameterized suite) - the only
substantive differences are the imported class/config and the
expected `order.source == "snappshop2"`.
...
def test_fetch_new_orders_sends_confirmed_auth_headers():
    ...
    adapter = SnappShop2Adapter(config=_CFG)
...
def test_fetch_new_orders_normalizes_real_history_sample():
    ...
    assert len(orders) == 1
    order = orders[0]
    assert order.source == "snappshop2"
