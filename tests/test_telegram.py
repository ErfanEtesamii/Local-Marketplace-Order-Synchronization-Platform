...
def test_is_configured_passes_current_proxy_to_client(monkeypatch):
    """proxy.txt's value (via src.proxy_config.get_current_proxy) must
    reach httpx.Client's `proxy` kwarg - this is the whole point of
    2026-09's hot-reloadable-proxy change. httpx.Client itself is
    mocked here (rather than only the HTTP transport, as respx does
    elsewhere in this file) specifically so we can assert on the
    constructor's call arguments."""
    monkeypatch.setenv("TELEGRAM_BOT_TOKEN", _TOKEN)
    monkeypatch.setenv("TELEGRAM_CHAT_ID", "775753176")
    monkeypatch.setattr(
        "src.telegram.get_current_proxy", lambda: "http://user:pass@10.0.0.1:8080"
    )
    ...
    with patch("src.telegram.httpx.Client", return_value=fake_client) as client_ctor:
        assert TelegramNotifier().is_configured() is True

    assert client_ctor.call_args.kwargs["proxy"] == "http://user:pass@10.0.0.1:8080"


def test_is_configured_passes_none_proxy_when_proxy_txt_empty(monkeypatch):
    """The common case (proxy.txt empty/absent) must pass proxy=None,
    not omit the kwarg or pass an empty string - httpx treats those
    differently."""
    monkeypatch.setenv("TELEGRAM_BOT_TOKEN", _TOKEN)
    monkeypatch.setenv("TELEGRAM_CHAT_ID", "775753176")
    monkeypatch.setattr("src.telegram.get_current_proxy", lambda: None)
    ...
    assert client_ctor.call_args.kwargs["proxy"] is None


@respx.mock
def test_transport_error_resets_configured_state(monkeypatch):
    """After a network/transport failure (dead proxy, filtered
    connection, etc.), _configured must flip back to False and
    _client to None, so the *next* call to is_configured() builds a
    brand-new httpx.Client - picking up whatever proxy.txt says by
    then - instead of being stuck forever on the client built the
    first time the process started (the exact bug the 2026-09 report
    diagnosed)."""
    monkeypatch.setenv("TELEGRAM_BOT_TOKEN", _TOKEN)
    monkeypatch.setenv("TELEGRAM_CHAT_ID", "775753176")
    _ok_get_me()
    ...
    notifier = TelegramNotifier()
    assert notifier.is_configured() is True
    assert notifier._configured is True

    respx.post(f"{_API}/getUpdates").mock(side_effect=httpx.ConnectError("proxy is dead"))

    with pytest.raises(TelegramError):
        notifier._request(notifier._client, "getUpdates", offset=-1, timeout=0)
...
@respx.mock
def test_http_status_error_does_not_reset_configured_state(monkeypatch):
    """A logical Telegram-side error (bad request, blocked chat, etc.)
    is NOT a network/proxy problem, so it must NOT reset the client -
    only httpx.TransportError does that (see
    test_transport_error_resets_configured_state)."""
    monkeypatch.setenv("TELEGRAM_BOT_TOKEN", _TOKEN)
    monkeypatch.setenv("TELEGRAM_CHAT_ID", "775753176")
    _ok_get_me()
    ...
    with pytest.raises(TelegramError):
        notifier._request(notifier._client, "sendMessage", chat_id=1, text="hi")

    assert notifier._configured is True
    assert notifier._client is live_client
